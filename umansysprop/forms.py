...
class SMILESListField(FormField):
    """
    Represents a compound input which accepts a list of SMILES strings
    representing chemical compounds, or a file containing one SMILES string per
    line. The field's data is returned as a sequence of OpenBabel Molecule
    objects
    ...
    def __init__(self, label=None, validators=None, compounds=None,
            separator='-', min_entries=0, max_entries=100, **kwargs):

        class ListForm(Form):
            smiles = FieldList(
                SMILESField(label, validators=validators, compounds=compounds),
                min_entries=min_entries, max_entries=max_entries)
            upload =FileField(label)

            def __init__(self, csrf_enabled=False, *args, **kwargs):
                super(ListForm, self).__init__(*args, csrf_enabled=False, **kwargs)
    ...
    def data(self):
        if self.form.upload.name in request.files:
            try:
                result = [smiles(s.strip()) for i, s in enumerate(
                    request.files[self.form.upload.name].read().splitlines())]
            except ValueError as e:
                raise ValueError('%s on line %d' % (str(e), i))
        else:
            result = self.form.smiles.data
        min_len = self.form.smiles.min_entries
        max_len = self.form.smiles.max_entries
    ...
    def __call__(self, **kwargs):
        if not len(self.form.smiles):
            self.form.smiles.append_entry()
        ...
        return tag.div(
            ...
            (tag.div(
                ...
                ) for field in self.form.smiles),
            id=self.id,
            data_toggle='fieldset',
            data_freeid=len(self.form.smiles)
            )
...
"""
        return literal('\n'.join(
            ...
            [field.scripts for field in self.form.smiles]
            ))
...
class FloatRangeField(FormField):
    ...
    def __init__(
        ...
        class RangeForm(Form):
            ...
            def __init__(self, csrf_enabled=False, *args, **kwargs):
                super(RangeForm, self).__init__(*args, csrf_enabled=False, **kwargs)
